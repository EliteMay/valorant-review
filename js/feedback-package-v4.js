window.VReviewFeedbackPackage = (() => {
  async function build(options = {}) {
    ...
    if (!file || !videoData) {
      throw new Error('元動画を読み込んでから提出用パッケージを作成してください。');
    }
    if (!detectionRun) {
      throw new Error('先にCombat Scene自動検出を実行してください。');
    }
    ...
    const tierCounts = countBy(corrected, scene => getReviewTier(scene));
    ...
    try {
      ...
      try {
        for (let i = 0; i < originalScenes.length; i++) {
          ...
          onProgress(
            progressPart(sheetIndex, totalSheets),
            `自動検出Scene ${i + 1}/${originalScenes.length} の確認画像を作成しています…`
          );
          const tier = getReviewTier(scene).toUpperCase();
          const blob = await createSceneSheet(
            source.video,
            scene,
            videoData.duration,
            `AUTO ${tier} ${String(i + 1).padStart(2, '0')}`
          );
          files.push({
            name: `auto-scenes/auto_${String(i + 1).padStart(2, '0')}.jpg`,
        ...
        for (let i = 0; i < corrected.length; i++) {
          ...
          onProgress(
            progressPart(sheetIndex, totalSheets),
            `修正後Scene ${i + 1}/${corrected.length} の確認画像を作成しています…`
          );
          ...
          const blob = await createSceneSheet(
            ...
            scene,
            videoData.duration,
            `${tier} ${label} ${String(i + 1).padStart(2, '0')}`
          );
          files.push({
            name: `corrected-scenes/corrected_${String(i + 1).padStart(2, '0')}.jpg`,
            data: new Uint8Array(await blob.arrayBuffer())
          });
          sheetIndex++;
        }
      ...
      imageWarning =
        `確認画像の生成に失敗したため、JSON中心のZIPを作成しました: ${error.message || error}`;
    ...
    const blob = buildStoreZip(
      files,
      value => onProgress(0.9 + value * 0.1, 'ZIPを作成しています…')
    );
  ...
  function buildReadme() {
    return [
      'VReview Detector Feedback Package v4',
      '',
      'このZIPは自動検出精度の改善用です。',
      'corrected-scenes.json の feedback_label がユーザーによる正解ラベルです。',
      'review_tier はDetectorが分類した表示優先度です。',
      '',
      'feedback_label:',
      '- kill: 欲しいキルScene',
      '- death: 欲しいデスScene',
      ...
      '',
      'weak_reason / needs_review / classifier_evidence もScene JSONへ保存します。',
      'detector-diagnostics.json にはBase / Refiner / Recall Guard / Candidate Classifierの診断を保存します。',
      '元動画そのものはZIPに含めていません。'
    ].join('\n');
  }
  ...
  async function createSceneSheet(video, scene, duration, label) {
    ...
    for (let i = 0; i < times.length; i++) {
      ...
      ctx.fillText(
        `${label} F${String(i + 1).padStart(2, '0')} ${formatTime(time)} ${marker}`,
        x + 8,
        y + 17
      );
    }
  ...
  function seekVideo(video, time) {
    return new Promise((resolve, reject) => {
      ...
      const fail = () => {
        cleanup();
        reject(new Error('提出用画像のフレーム取得に失敗しました。'));
      };
  ...
  function cleanScenes(scenes) {
    return (scenes || []).map((scene, index) => ({
      ...
      confidence: Number.isFinite(scene.confidence)
      ...
      classifier_evidence: cleanEvidence(scene.classifierEvidence),
  ...
  function cleanEvidence(value) {
    ...
    for (const [key, raw] of Object.entries(value)) {
      if (Number.isFinite(raw)) output[key] = round(raw, 4);
  ...
  function getReviewTier(scene) {
    ...
    if (scene?.reviewTier === 'weak') return 'weak';
    if (scene?.reviewTier === 'primary') return 'primary';
    if (scene?.needsReview) return 'weak';
    return 'primary';
  }
