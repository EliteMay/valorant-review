window.VReviewFeedbackPackage = (() => {
  ...
  function buildReadme() {
    return [
      ...
      'auto-scenes.json と corrected-scenes.json の差、detector-diagnostics.json、確認画像を比較して検出ロジックを改善します。',
      '',
      '元動画そのものはZIPに含めていません。'
    ].join('\n');
  }
  ...
  function addText(files, name, text) { files.push({ name, data: new TextEncoder().encode(text) }); }
  ...
  function buildStoreZip(files, onProgress = () => {}) {
    ...
    files.forEach((file, index) => {
      ...
      lv.setUint16(10, dos.time, true); lv.setUint16(12, dos.date, true); lv.setUint32(14, crc, true); lv.setUint32(18, data.length, true);
      lv.setUint32(22, data.length, true); lv.setUint16(26, nameBytes.length, true); lv.setUint16(28, 0, true); local.set(nameBytes, 30);
      ...
      cv.setUint16(12, dos.time, true); cv.setUint16(14, dos.date, true); cv.setUint32(16, crc, true); cv.setUint32(20, data.length, true); cv.setUint32(24, data.length, true);
  ...
  function dosDateTime(date) { const year = Math.max(1980, date.getFullYear()); return { time: ((date.getHours() & 31) << 11) | ((date.getMinutes() & 63) << 5) | (Math.floor(date.getSeconds() / 2) & 31), date: (((year - 1980) & 127) << 9) | (((date.getMonth() + 1) & 15) << 5) | (date.getDate() & 31) }; }

  function cleanScenes(scenes) {
    return (scenes || []).map((scene, index) => ({ index: index + 1, id: scene.id || null, start: round(scene.start, 3), end: round(scene.end, 3), duration: round(Number(scene.end || 0) - Number(scene.start || 0), 3), confidence: Number.isFinite(scene.confidence) ? round(scene.confidence, 4) : null, source: scene.source || null, fps: scene.fps || 'auto' }));
  }
  function progressPart(index, total) { return total ? 0.08 + (index / total) * 0.78 : 0.15; }
  function download(blob, filename) { const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = filename; document.body.appendChild(a); a.click(); a.remove(); setTimeout(() => URL.revokeObjectURL(url), 3000); }
  function formatTime(seconds) { const m = Math.floor(seconds / 60); const s = seconds - m * 60; return `${String(m).padStart(2, '0')}:${s.toFixed(3).padStart(6, '0')}`; }
  function sanitizeBaseName(value) { return String(value || '').replace(/[^a-zA-Z0-9_-]+/g, '_').replace(/^_+|_+$/g, '').slice(0, 40); }
  function round(value, digits) { const n = Number(value || 0), p = 10 ** digits; return Math.round(n * p) / p; }
